...
public:
    ...
    void setup() {
        ...
        StorageHelper::begin();
        uint32_t savedStartTime = StorageHelper::getUInt("patchStart", 0);
        uint8_t savedPatchState = StorageHelper::getUChar("patchState", 0);
        uint32_t savedElapsedTime = StorageHelper::getUInt("elapsedTime", 0);
        ...
        GATTServer::gInstance = &gattServer;
        gattServer.onWriteRequest = handleWriteRequestStatic;
        gattServer.onConnectCallback = handleConnectStatic;
        gattServer.onDisconnectCallback = handleDisconnectStatic;
...
private:
    ...
    void update() {
        ...
        if (patchStateDirty) {
            ...
            StorageHelper::putUChar("patchState", static_cast<uint8_t>(patchState));
            StorageHelper::putUInt("patchStart", patchStartTime);
            StorageHelper::putUInt("elapsedTime", totalElapsedTime);
        }
    ...
    void handleStopPatchRequest(const uint8_t* data, uint8_t len, uint8_t seqNum) {
        ...
        StorageHelper::remove("patchStart");
        StorageHelper::remove("patchState");
        StorageHelper::remove("elapsedTime");
