...
public class M640GKitPumpManager: DeviceManager {
    public static let pluginIdentifier = "M640GKit"
    public var localizedTitle: String {
        LocalizedString("M640GKit TouchCare Nano", comment: "Generic title of the M640GKit pump manager")
    }

    public let managerIdentifier: String = "M640GKit"

    private let log = M640GKitLogger(category: "M640GKitPumpManager")

    public let pumpDelegate = WeakSynchronizedDelegate<PumpManagerDelegate>()
    private let statusObservers = WeakSynchronizedSet<PumpManagerStatusObserver>()

    public var state: M640GKitPumpState
    var oldState: M640GKitPumpState
    public var rawState: PumpManager.RawStateValue {
        state.rawValue
    }

    let bluetooth: BluetoothManager
    init(state: M640GKitPumpState) {
        self.state = state
        oldState = M640GKitPumpState(rawValue: state.rawValue)
    ...
    public required convenience init?(rawState: RawStateValue) {
        self.init(state: M640GKitPumpState(rawValue: rawState))
    }
    ...
    public var supportedBasalRates: [Double] {
        guard !state.pumpSN.isEmpty else {
            return M640GKitPumpManager.onboardingSupportedBasalRates
        }

        // 300U -> 0.05-30U
        // 200U -> 0.05-25U
        return state.pumpName.contains("300U") ? M640GKitPumpManager.onboardingSupportedBasalRates : (0 ... 500)
            .map { Double($0) / 20 }
    }
    ...
    private func device(_ state: M640GKitPumpState) -> HKDevice {
        HKDevice(
            name: state.pumpName,
            manufacturer: "M640GKit",
            model: state.model,
            hardwareVersion: nil,
            firmwareVersion: nil,
...
public extension M640GKitPumpManager {
    ...
    private func status(_ state: M640GKitPumpState) -> PumpManagerStatus {
        PumpManagerStatus(
    ...
    func createBolusProgressReporter(reportingOn: DispatchQueue) -> (any LoopKit.DoseProgressReporter)? {
        if let doseEntry = state.doseEntry {
            return M640GKitDoseProgressReporter(
                pumpManager: self,
                dose: doseEntry,
    ...
    func enactBolus(
        ...
        guard state.bolusState == .noBolus else {
            ...
            completion(.deviceState(M640GKitConnectError.isBolussing))
            return
        }
    ...
    func activatePatch(_ completion: @escaping (M640GKitActivatePatchResult) -> Void) {
    ...
    func deactivatePatch(_ completion: @escaping (M640GKitDeactivatePatchResult) -> Void) {
    ...
    func updatePatchSettings(completion: @escaping (M640GKitUpdatePatchResult) -> Void) {
    ...
    func notifyStateDidChange() {
        DispatchQueue.main.async {
            ...
            self.oldState = M640GKitPumpState(rawValue: self.state.rawValue)
        }
    }
