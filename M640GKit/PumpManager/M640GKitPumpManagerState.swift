...
public struct M640GKitPumpManagerState: RawRepresentable, Equatable {
    ...
    public init(rawValue: RawValue) {
        ...
        encryptionMode = rawValue["encryptionMode"] as? UInt8 ?? M640GKitPumpManagerState.getEncryptionMode(hwModel)
        ...
        if let rawBasalDeliveryOrdinal = rawValue["basalDeliveryOrdinal"] as? M640GKitBasal.RawValue {
            basalDeliveryOrdinal = M640GKitBasal(rawValue: rawBasalDeliveryOrdinal) ?? .active
        } else {
    ...
    public var basalDeliveryOrdinal: M640GKitBasal = .active
    ...
    /// Allows M640GKit to automaticly sync the time every evening
...
extension M640GKitPumpManagerState: CustomDebugStringConvertible {
    public var debugDescription: String {
        [
            "## M640GKitPumpManagerState - \(Date.now)",
