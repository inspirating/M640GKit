...
public class M640GPumpManager: DeviceManager {
    public static let pluginIdentifier = "M640G"
    public let localizedTitle: String = "M640G Nano"

    public let managerIdentifier: String = "M640G"

    private let log = M640GLogger(category: "M640GPumpManager")

    public let pumpDelegate = WeakSynchronizedDelegate<PumpManagerDelegate>()
    private let statusObservers = WeakSynchronizedSet<PumpManagerStatusObserver>()

    public var state: M640GPumpState
    var oldState: M640GPumpState
    public var rawState: PumpManager.RawStateValue {
        state.rawValue
    }

    let bluetooth: BluetoothManager
    init(state: M640GPumpState) {
        self.state = state
        oldState = M640GPumpState(rawValue: state.rawValue)
    ...
    public required convenience init?(rawState: RawStateValue) {
        self.init(state: M640GPumpState(rawValue: rawState))
    }
    ...
    public static var onboardingSupportedMaximumBolusVolumes: [Double] {
        M640GPumpManager.onboardingSupportedBolusVolumes
    }
    ...
    public var supportedBasalRates: [Double] {
        guard !state.pumpSN.isEmpty else {
            return M640GPumpManager.onboardingSupportedBasalRates
        }

        // 300U -> 0.05-30U
        // 200U -> 0.05-25U
        return state.pumpName.contains("300U") ? M640GPumpManager.onboardingSupportedBasalRates : (0 ... 500)
            .map { Double($0) / 20 }
    }
    ...
    public var maximumBasalScheduleEntryCount: Int {
        M640GPumpManager.onboardingMaximumBasalScheduleEntryCount
    }
    ...
    private func device(_ state: M640GPumpState) -> HKDevice {
        HKDevice(
            name: state.pumpName,
            manufacturer: "M640G",
            model: state.model,
...
public extension M640GPumpManager {
    ...
    private func status(_ state: M640GPumpState) -> PumpManagerStatus {
        PumpManagerStatus(
    ...
    func createBolusProgressReporter(reportingOn: DispatchQueue) -> (any LoopKit.DoseProgressReporter)? {
        if let doseEntry = state.bolusDose {
            return M640GDoseProgressReporter(
                pumpManager: self,
                dose: doseEntry,
    ...
    func enactBolus(
        ...
        guard state.bolusState == .noBolus else {
            ...
            completion(.deviceState(M640GConnectError.isBolussing))
            return
        }
    ...
    func syncDeliveryLimits(
        ...
        log.warning("Skipping sync delivery limits (not supported by M640G)")
        completion(.success(limits))
    }

    func primePatch(_ completion: @escaping (M640GPrimePatchResult) -> Void) {
    ...
    func activatePatch(_ completion: @escaping (M640GActivatePatchResult) -> Void) {
    ...
    func deactivatePatch(_ completion: @escaping (M640GDeactivatePatchResult) -> Void) {
    ...
    func updatePatchSettings(completion: @escaping (M640GUpdatePatchResult) -> Void) {
    ...
    func notifyStateDidChange() {
        DispatchQueue.main.async {
            ...
            self.oldState = M640GPumpState(rawValue: self.state.rawValue)
        }
    }
    ...
    private func ensureConnectedAndActive(_ completionAsync: @escaping (M640GConnectError?) async -> Void) {
        guard state.pumpState.rawValue >= PatchState.active.rawValue else {
