...
public class M640GPumpManager: RileyLinkPumpManager {
    ...
    public init(state: M640GPumpManagerState, rileyLinkDeviceProvider: RileyLinkDeviceProvider, pumpOps: PumpOps? = nil, dateGenerator: @escaping () -> Date = Date.init) {
        ...
        self.hkDevice = HKDevice(
            name: M640GPumpManager.pluginIdentifier,
    ...
    public required convenience init?(rawState: PumpManager.RawStateValue) {
        guard let state = M640GPumpManagerState(rawValue: rawState),
    ...
    public let stateObservers = WeakSynchronizedSet<M640GPumpManagerStateObserver>()

    public var state: M640GPumpManagerState {
        return lockedState.value
    }
    private let lockedState: Locked<M640GPumpManagerState>
    
    private func setState(_ changes: (_ state: inout M640GPumpManagerState) -> Void) -> Void {
        return setStateWithResult(changes)
    }
    
    private func mutateState(_ changes: (_ state: inout M640GPumpManagerState) -> Void) -> M640GPumpManagerState {
        return setStateWithResult({ (state) -> M640GPumpManagerState in
            changes(&state)
            return state
        })
    }
    
    private func setStateWithResult<ReturnType>(_ changes: (_ state: inout M640GPumpManagerState) -> ReturnType) -> ReturnType {
        var oldValue: M640GPumpManagerState!
    ...
    public var recents: M640GPumpManagerRecents {
        get {
    ...
    private let lockedRecents = Locked(M640GPumpManagerRecents())
    ...
    public let log = OSLog(category: "M640GPumpManager")
...
extension M640GPumpManager {
        ...
        rileyLinkDeviceProvider.getDevices { (devices) in
            ...
            self.pumpOps.runSession(withName: sessionName, using: device) { (session) in
                do {
                    ...
                    completion(M640GPumpManagerError.commsError(error as! PumpCommandError))
                }
            }
        }
    }
    ...
    public func buildPumpStatusHighlight(for state: M640GPumpManagerState, recents: M640GPumpManagerRecents, andDate date: Date) -> PumpStatusHighlight? {
    ...
    private func reconcilePendingDosesWith(_ events: [NewPumpEvent], fetchedAt: Date) -> [NewPumpEvent] {
        ...
        return setStateWithResult { (state) -> [NewPumpEvent] in
            ...
            let result = M640GPumpManager.reconcilePendingDosesWith(events, reconciliationMappings: state.reconciliationMappings, pendingDoses: allPending)
    ...
    private func fetchPumpHistory(_ completion: @escaping (_ error: Error?) -> Void) {
        guard let insulinType = insulinType else {
            completion(PumpManagerError.configuration(M640GPumpManagerError.insulinTypeNotConfigured))
            return
        }
    ...
    private func storePendingPumpEvents(forceFinalization: Bool = false, _ completion: @escaping (_ error: M640GPumpManagerError?) -> Void) {
        // Must be called from the sessionQueue
        ...
        self.pumpDelegate.notify({ (delegate) in
            ...
            delegate.pumpManager(self, hasNewPumpEvents: events, lastReconciliation: self.state.lastReconciliation, replacePendingEvents: true) { (error) in
                ...
                if let error = error {
                    ...
                    completion(M640GPumpManagerError.storageFailure)
                } else {
                    completion(nil)
                }
            }
...
// MARK: - PumpManager
extension M640GPumpManager: PumpManager {
    
    public static let localizedTitle = LocalizedString("M640G", comment: "Generic title of the M640G pump manager")

    public var localizedTitle: String {
        return String(format: LocalizedString("M640G %@", comment: "Pump title (1: model number)"), state.pumpModel.rawValue)
    }
    ...
    private func lastSync(for state: M640GPumpManagerState, recents: M640GPumpManagerRecents) -> Date? {
    ...
    private func status(for state: M640GPumpManagerState, recents: M640GPumpManagerRecents) -> PumpManagerStatus {
            ...
            self.pumpOps.runSession(withName: "Get Pump Status", using: device) { (session) in
                do {
                    ...
                    guard var date = status.clock.date else {
                        ...
                        throw PumpManagerError.configuration(M640GPumpManagerError.noDate)
                    }
    ...
    public func enactBolus(units: Double, activationType: BolusActivationType, completion: @escaping (PumpManagerError?) -> Void) {
        ...
        guard let insulinType = insulinType else {
            completion(.configuration(M640GPumpManagerError.insulinTypeNotConfigured))
            return
        }
        ...
        pumpOps.runSession(withName: "Bolus", usingSelector: rileyLinkDeviceProvider.firstConnectedDevice) { (session) in
            ...
            if let unfinalizedBolus = self.state.unfinalizedBolus {
                guard unfinalizedBolus.isFinished else {
                    completion(.deviceState(M640GPumpManagerError.bolusInProgress))
                    return
                }
            ...
            if case .suspended = self.state.suspendState {
                guard activationType.isAutomatic == false else {
                    ...
                    completion(.deviceState(M640GPumpManagerError.pumpSuspended))
                    return
                }
                do {
    ...
    public func cancelBolus(completion: @escaping (PumpManagerResult<DoseEntry?>) -> Void) {
        ...
        guard let insulinType = insulinType else {
            completion(.failure(.configuration(M640GPumpManagerError.insulinTypeNotConfigured)))
            return
        }
    ...
    public func createBolusProgressReporter(reportingOn dispatchQueue: DispatchQueue) -> DoseProgressReporter? {
        if let bolus = self.state.unfinalizedBolus, !bolus.isFinished {
            return M640GDoseProgressEstimator(dose: DoseEntry(bolus), pumpModel: state.pumpModel, reportingQueue: dispatchQueue)
        }
        return nil
    }
        ...
        pumpOps.runSession(withName: "Set time", usingSelector: rileyLinkDeviceProvider.firstConnectedDevice) { (session) in
            do {
                ...
                guard self.state.unfinalizedBolus == nil else {
                    throw PumpManagerError.configuration(M640GPumpManagerError.bolusInProgress)
                }
...
extension M640GPumpManager: PumpOpsDelegate {
...
extension M640GPumpManager: CGMManager {
