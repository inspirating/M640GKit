...
private let log = OSLog(category: "M640GPumpMessageSender")

struct M640GPumpMessageSender: PumpMessageSender {
    ...
    func send(_ msg: PumpMessage) throws {
        do {
            try commandSession.send(M640GPacket(outgoingData: msg.txData).encodedData(), onChannel: 0, timeout: 0)
    ...
    func sendAndListen(_ message: PumpMessage, repeatCount: Int, timeout: TimeInterval, retryCount: Int) throws -> PumpMessage {
        ...
        guard let packet = M640GPacket(encodedData: rfPacket.data) else {
    ...
    func sendAndListenForPacket(_ message: PumpMessage, repeatCount: Int, timeout: TimeInterval, retryCount: Int) throws -> RFPacket {
        ...
        do {
            packet = try commandSession.sendAndListen(M640GPacket(outgoingData: message.txData).encodedData(), repeatCount: repeatCount, timeout: timeout, retryCount: retryCount)
        } catch let error as LocalizedError {
            throw PumpOpsError.deviceError(error)
        }
