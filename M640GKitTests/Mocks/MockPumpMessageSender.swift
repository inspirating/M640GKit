...
class MockPumpMessageSender: PumpMessageSender {
    ...
    func sendAndListen(_ data: Data, repeatCount: Int, timeout: TimeInterval, retryCount: Int) throws -> RFPacket {
        guard let decoded = M640GPacket(encodedData: data),
        ...
        if let responseArray = responses[messageType] {
            ...
            // .pumpAck from 636781 ?
            let packet = M640GPacket(encodedData: Data(hexadecimalString: "a969a39966b1566555b235")!)!
            response = PumpMessage(rxData: packet.data)!
        }

        var encoded = M640GPacket(outgoingData: response.txData).encodedData()
    ...
    func sendAndListenForPacket(_ message: PumpMessage, repeatCount: Int, timeout: TimeInterval, retryCount: Int) throws -> RFPacket {
        ...
        do {
            packet = try sendAndListen(M640GPacket(outgoingData: message.txData).encodedData(), repeatCount: repeatCount, timeout: timeout, retryCount: retryCount)
    ...
    func send(_ msg: M640GKit.PumpMessage) throws {
        // Do nothing
    }
