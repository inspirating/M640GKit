...
final class ReconciliationTests: XCTestCase {
    ...
    func testPendingDoseUpdatesWithActualDeliveryFromHistoryDose() {
        ...
        let result = M640GPumpManager.reconcilePendingDosesWith([bolusEvent], reconciliationMappings: [:], pendingDoses: [unfinalizedBolus])
    ...
    func testReconciledDosesShouldNotAppearInReturnedPumpEvents() {
        ...
        let result = M640GPumpManager.reconcilePendingDosesWith([bolusEvent], reconciliationMappings: reconciliationMappings, pendingDoses: [])
