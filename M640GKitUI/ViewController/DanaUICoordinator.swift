...
class DanaUICoordinator: UINavigationController, PumpManagerOnboarding, CompletionNotifying, UINavigationControllerDelegate {
    ...
    )
    {
        if pumpManager == nil, pumpManagerSettings == nil {
            self.pumpManager = M640GKitPumpManager(state: M640GKitPumpManagerState(rawValue: [:]))
        } else if pumpManager == nil, pumpManagerSettings != nil {
            let basal = M640GKitPumpManagerState.convertBasal(pumpManagerSettings!.basalSchedule.items)
            self.pumpManager = M640GKitPumpManager(state: M640GKitPumpManagerState(basalSchedule: basal))
        } else {
    ...
    private func viewControllerForScreen(_ screen: DanaUIScreen) -> UIViewController {
        ...
        case .bolusSpeedScreen:
            ...
            let view = M640GKitPumpSpeed(next: next)
        ...
        case .deviceScanningScreen:
            ...
            let viewModel = M640GKitScanViewModel(pumpManager, nextStep: stepFinished)
            return hostingController(rootView: M640GKitScanView(viewModel: viewModel))
        case .setupComplete:
            let nextStep: () -> Void = {
            ...
            let view = M640GKitSetupCompleteView(
                finish: nextStep,
            ...
            return hostingController(rootView: view)
        case .settings:
            let view = M640GKitSettingsView(
                viewModel: M640GKitSettingsViewModel(pumpManager, stepFinished),
                supportedInsulinTypes: allowedInsulinTypes,
                imageName: pumpManager?.state.getDanaPumpImageName() ?? "danai"
            )
            return hostingController(rootView: view)
