...
struct M640GKitSettingsView: View {
    @Environment(\.openURL) private var openURL
    @Environment(\.guidanceColors) private var guidanceColors
    @Environment(\.dismissAction) private var dismiss
    @Environment(\.insulinTintColor) var insulinTintColor

    @ObservedObject var viewModel: M640GKitSettingsViewModel
    ...
    var body: some View {
        List {
            Section {
                HStack {
                    Spacer()
                    Image(uiImage: UIImage(named: imageName, in: Bundle(for: M640GKitHUDProvider.self), compatibleWith: nil)!)
                        .resizable()
                        .scaledToFit()
                        .padding(.horizontal)
                        .frame(height: 200)
            ...
            Section {
                ...
                Button(action: {
                    ...
                    HStack {
                        Text(LocalizedString("Sync pump data", comment: "M640GKit sync pump"))
                        Spacer()
                ...
                if viewModel.isUsingContinuousMode {
                    if !viewModel.isConnected {
                        Button(action: {
                            ...
                            HStack {
                                Text(LocalizedString("Reconnect to pump", comment: "M640GKit reconnect"))
                                Spacer()
                        ...
                        Button(action: {
                            ...
                            HStack {
                                Text(LocalizedString("Disconnect from pump", comment: "M640GKit disconnect"))
                                Spacer()
                                if viewModel.isTogglingConnection {
                                    ActivityIndicator(isAnimating: .constant(true), style: .medium)
                                }
            ...
            Section(header: SectionHeader(label: LocalizedString(
                "Configuration",
                comment: "The title of the configuration section in M640GKit settings"
            )))
                {
                    ...
                    NavigationLink(destination: M640GKitSettingsPumpSpeed(
            ...
            Section(header: SectionHeader(label: LocalizedString(
                "Pump information",
                comment: "The title of the pump information section in M640GKit settings"
            ))) {
                HStack {
            ...
            Section(header: SectionHeader(label: LocalizedString(
                "Pump time",
                comment: "The title of the pump time section in M640GKit settings"
            ))) {
                HStack {
            ...
            Section {
                Button(LocalizedString("Share Dana pump logs", comment: "M640GKit share logs")) {
    ...
    var continuousConnectionStatusText: some View {
        if viewModel.isTogglingConnection {
            if viewModel.isConnected {
                return Text(LocalizedString("Disconnecting...", comment: "M640GKit disconnecting"))
            } else {
                return Text(LocalizedString("Reconnecting...", comment: "M640GKit reconnecting"))
            }
        } else {
            if viewModel.isConnected {
                return Text(LocalizedString("Connected", comment: "M640GKit connected"))
            } else {
                return Text(LocalizedString("Disconnected", comment: "M640GKit disconnected"))
            }
        }
    }
...
#Preview {
    M640GKitSettingsView(
        viewModel: M640GKitSettingsViewModel(nil, nil),
