...
inline uint8_t crc8Calculate(const uint8_t* data, size_t length) {
    uint8_t crc = 0;
...
inline bool crc8Verify(const uint8_t* data, size_t length, uint8_t expectedCrc) {
    return crc8Calculate(data, length) == expectedCrc;
}
