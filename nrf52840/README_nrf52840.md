# M640GKit 泵模拟器 — nRF52840 (NimBLE 版)

本目录是把 `ESP32/` 版本的胰岛素泵模拟器移植到 **Nordic nRF52840** 的产物,使用 **NimBLE-Arduino** 库实现低功耗蓝牙通信。
...
|--------|-------|-------------------|
| BLE 栈 | `<BLEDevice.h>` (Arduino-ESP32 BLE) | `<NimBLEDevice.h>` (NimBLE-Arduino) → 重写 `gatt_server.h` |
...
### NimBLE vs Adafruit Bluefruit 功耗对比

| 指标 | Adafruit Bluefruit | NimBLE |
|------|-------------------|--------|
| 广播间隔 | 20ms/30ms (持续高速) | 1s/2s (省电模式) |
| 连接时平均电流 | ~15mA | ~5mA |
| 空闲时平均电流 | ~15mA | ~1mA |
| 固件大小 | 较大 | 较小 |
...
| 文件 | 来源 | 说明 |
|------|------|------|
| `nrf52840.ino` | 改自 `ESP32.ino` | 入口,删 esp_reset/ESP.getFreeHeap,NimBLE 版 |
| `pump_simulator.h` | 改自 ESP32 版 | 3 处改动:Preferences include / FreeRTOS include / STEP_PIN / 栈大小 |
| `gatt_server.h` | **重写** | NimBLE 实现,公共 API 与 ESP32 版完全一致 |
...
**需要安装 NimBLE-Arduino 库**：

1. Arduino IDE → 项目 → 包含库 → 管理库...
2. 搜索 `NimBLE-Arduino` (作者: h2zero)
3. 安装最新版本

其他依赖：
- `InternalFS` / `Adafruit_LittleFS`(持久化) - 由 Adafruit nRF52 板卡包自带
- FreeRTOS(任务/信号量) - 由 Adafruit nRF52 板卡包自带
...
09:09:03.989 -> [BLE][I] 收到命令: SET_BOLUS len=9 seq=11 pkg=0
09:09:03.989 -> [BLE][I] 处理完整命令: SET_BOLUS (10 bytes)
09:09:03.989 -> [BLE][I] === 设置大剂量 ===
09:09:03.989 -> [BLE][I] 大剂量类型: 1, 剂量: 3.00U
09:09:03.989 -> [BLE][W] 超过每小时最大胰岛素限制: 22.00U > 20.00U
09:09:03.989 -> [BLE][W] 发送错误响应: cmd=SET_BOLUS code=0x30a (超过每小时最大量)
