/*
================================================================================
M640GKit 泵模拟器核心 (C++ 版本 - nRF52840 + NimBLE)
================================================================================

该程序模拟 M640G 胰岛素泵, 作为 GATT Server 运行,
供 iOS Loop app 或其他 BLE 客户端连接和通信

移植自 ESP32/ESP32.ino, 业务逻辑零改动 (见 ESP32/pump_simulator.h)。
平台差异 (仅此文件层面):
  - #include "preferences_nrf52.h" → #include "preferences_nrf52.h"
  - #include "semphr.h" → nRF52 FreeRTOS 适配
  - STEP_PIN = 7 (ESP32) → 17 (Nice!Nano P0.17)
  - esp_mac.h 移除 (nRF52 使用 NimBLE 设备地址)

对应 ESP32: pump_simulator.h
================================================================================
*/
...
static constexpr int STEP_PIN = 17;  // Nice!Nano: P0.17 (D2 排针)
